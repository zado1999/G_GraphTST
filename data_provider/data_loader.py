...
class Dataset_Group(Dataset):
    ...
    def __read_data__(self):
        ...
        df_raw = pd.read_csv(
            os.path.join(self.root_path, self.data_path), index_col=0, parse_dates=True
        )

        if self.features == "S":
            raise NotImplementedError

        df_train = df_raw.loc[(df_raw["group"] <= 20) & (df_raw.index.year <= 2022)]
        df_val = df_raw.loc[
            (df_raw["group"] > 20)
            & (df_raw.index.year <= 2022)
            & (df_raw.index.year >= 2020)
        ]
        df_test = df_raw.loc[(df_raw["group"] > 20) & (df_raw.index.year >= 2023)]
        ...
        if self.scale:
            ...
            df_data = pd.DataFrame(data, columns=df_data.columns, index=df_data.index)
        else:
            data = df_data.values
        ...
        for group in df_data["group"].unique():
            ...
            # 时间序列滑窗
            for i in range(len(df_group) - self.seq_len - self.pred_len + 1):
                data_all.append(
                    df_group.iloc[i : i + self.seq_len + self.pred_len].values
                )
        data_all = np.array(data_all, dtype=np.float32)
        self.data_x = data_all[:, : self.seq_len]
        self.data_y = data_all[:, self.seq_len - self.label_len :]

        df_stamp = pd.DataFrame(df_data.index, columns=["date"])
        if self.timeenc == 0:
        ...
        data_stamp = []
        df_stamp["group"] = df_data["group"].values
        for group in df_stamp["group"].unique():
            df_group = df_stamp[df_stamp["group"] == group].drop(
                ["date", "group"], axis=1
            )
            for i in range(len(df_group) - self.seq_len - self.pred_len + 1):
                data_stamp.append(
        ...
        data_stamp = np.array(data_stamp, dtype=np.float32)

        self.data_mark_x = data_stamp[:, : self.seq_len]
        self.data_mark_y = data_stamp[:, self.seq_len - self.label_len :]
    ...
    def __getitem__(self, index):
        ...
        seq_x_mark = self.data_mark_x[index]
        seq_y_mark = self.data_mark_y[index]
